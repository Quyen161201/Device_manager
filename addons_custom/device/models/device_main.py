...
class DeviceMain(models.Model):
    ...
    #bảo dưỡng
    maintenance_deadline = fields.Integer('Thời hạn bảo dưỡng')
    maintenance_deadline_type = fields.Selection([('day', 'Ngày'), ('month', 'Tháng'), ('year', 'Năm')], 'Loại', default='month')
    last_maintenance = fields.Date('Bảo dưỡng lần cuối', readonly=True)
    maintenance_expire_date = fields.Date('Ngày đến hạn', readonly=True)
    maintenance_status = fields.Text('Tình trạng', readonly=True)

    @api.onchange('maintenance_deadline')
    def _maintenance_deadline(self):
        current_date = datetime.now()
        type = self.maintenance_deadline_type
        if type == 'day':
            new_date = current_date + timedelta(days=self.maintenance_deadline)
            self.maintenance_expire_date = new_date
            self.maintenance_status = f'Thời hạn bảo dưỡng còn {self.maintenance_deadline} ngày'
        if type == 'month':
            new_date = current_date + relativedelta(months=+self.maintenance_deadline)
            self.maintenance_status = f'Thời hạn bảo dưỡng còn {abs(new_date-current_date).days} ngày'
            self.maintenance_expire_date = new_date
        if type == 'year':
            new_date = current_date + relativedelta(years=+self.maintenance_deadline)
            self.maintenance_status = f'Thời hạn bảo dưỡng còn {abs(new_date - current_date).days} ngày'
            self.maintenance_expire_date = new_date
